...
class wc_quick_donation{
	...
	public function wc_qd_form(){
		...
		$donate = isset($woocommerce->session->jc_donation) ? floatval($woocommerce->session->jc_donation) : "10";
	...
	public function get_projects_list(){
			...
			if($projects_db){
				...
				foreach($projects_db as $proj){
					if($proj == "Sostieni Reti Senza Frontiere") {
						$project_list .= '<option value="'.$proj.'" selected> '.$proj.'</option>';
					} else {
						if(count($projects_db) > 1 && !in_array("Sostieni Reti Senza Frontiere", $projects_db)) {
							$project_list .= '<option value="" >Seleziona un progetto...</option>';
						}
				...
				<div class="input-group">
					<label for="donation_project">Progetto</label>
					<select id="donation_project" name="projects" class="form-control"><?php print $project_list; ?></select>
					<span class="help-block">
						<a href="/associazione/progetti" target="blank" class="pull-right btn btn-link"><span class="um-faicon-question-circle"></span> <small>Maggiori informazioni sui progetti attivi</small></a>
					</span>
				</div>
				<?php
			}
